...
import {Col, Grid, MainContainer, Row} from "@sketchpixy/rubix/lib/index";
import Sidebar from '../common/sidebar';
import {authenticated, loginSuccess, setRedirectUrl, updateUserData} from "./redux/loginAction";
import _ from 'lodash';
import HeaderWithDatePickerFilter from "../common/HeaderWithDatePickerFilter";
...
export default class AuthenticationWithDatePickerFilter extends React.Component {
    ...
    authAndRoute=(props)=>{
        ...
        return !((routeAllowedRoles && userRole && _.intersection(routeAllowedRoles, userRole).length === 0) || !userRole);
    ...
    componentWillMount() {
        ...
        if (!user) {
            ...
            if(hash)
                this.props.isLoggedInAction({authHash: hash,history:this.props, onSuccessRedirect: currentURL, onFailureRedirect: "/login"});
            else
    ...
    render() {
        ...
        return <MainContainer {...this.props}>
            ...
            <HeaderWithDatePickerFilter history={this.props}/>
            <div id='body'>
                <Grid>
                    <Row>
                        <Col xs={12}>
